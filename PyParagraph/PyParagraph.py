...
file_to_load = os.path.join("Resources", "Adam Wayne.txt")
...
avg_sentence_length_w = sum(words_per_sentence) / float(len(words_per_sentence))
    
# Generate Paragraph Analysis Output
paragraph_output = (
        f"\nParagraph Analysis\n"
        f"-----------------\n"
        f"Apporximate Word Count: {word_count}\n"
        f"Approximate Sentence Count: {sentence_count}\n"
        f"Average Letter Count: {avg_letter_count:.1f}\n"
        f"Average Sentence Length:{avg_sentence_length_w}\n")
